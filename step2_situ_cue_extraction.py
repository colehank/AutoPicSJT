...
# def process_situ_character(traits, save = True, results_root='results', n_repeat = 30):
#     """
#     Process situational character data for given traits and item range.

#     Args:
#         traits (list): List of traits to process (e.g., ['O', 'C', 'E', 'A', 'N']).
#         results_root (str): Root directory for results.

#     Returns:
#         dict: Processed situational character data.
#     """
#     output_dir=f'{results_root}/situ_character'
#     os.makedirs(output_dir, exist_ok=True)
#     all_res = []
...
#             if trait not in all_res[i]:
#                 all_res[i][trait] = {}
#             for item_id in tqdm(range(0, 22), desc=f'Running items for {trait}', position=2, leave=False):
#                 while True:
#                     try:
#                         res = gen_situ_character(trait, str(item_id), results_root=results_root)
#                         break
#                     except Exception as e:
#                         msg.fail(f'Failed to process {trait}_{item_id} after {n_repeat} attempts: {e}')
#                         continue
#                 all_res[i][trait][item_id] = res
#     all_res[i][trait][item_id] = res
#     all_res = merge_dfs(all_res)
...
if __name__ == '__main__':
    ...
    thre = 4  # threshold for cues defined as situation cues
    highlight = 'red'
