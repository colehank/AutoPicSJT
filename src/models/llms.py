...
class TempletLLM():
    ...
    def _repr_html_(self):
        if self.task is None:
            ...
            params = {task: self.prompt_manager._templates[task]["required_params"] for task in tasks}
            df = pd.DataFrame(params.items(), columns=["Task", "Required Parameters"])
    ...
    def _check_task(self, task):
        if task not in self.prompt_manager._templates:
            raise ValueError(f"Invalid task name: {task}. Available tasks: {self.tasks}")
