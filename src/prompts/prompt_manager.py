...
class PromptTemplateManager:
    ...
    def get_templete(self, task_name: str) -> list[dict[str, str]] | None:
    ...
    def make_prompt(self, task_name: str, passage: str, **kwargs) -> list[dict[str, str]]:
        ...
        if missing_params:
            raise ValueError(
                f'Missing required parameters for {task_name}: {missing_params}',
            )

        # Process named_entities if provided but as a list
        if 'named_entities' in params and isinstance(params['named_entities'], list):
            params['named_entities'] = json.dumps(
                params['named_entities'], ensure_ascii=False,
            )
